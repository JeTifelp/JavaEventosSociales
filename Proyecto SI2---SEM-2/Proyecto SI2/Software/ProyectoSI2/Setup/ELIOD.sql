USE [master]
GO
IF SUSER_SID(N'ELIOD') IS NOT NULL
DROP LOGIN [ELIOD]
GO
EXEC msdb.dbo.sp_delete_database_backuphistory @database_name = N'ELIOD'
GO
ALTER DATABASE [ELIOD] SET  SINGLE_USER WITH ROLLBACK IMMEDIATE
IF EXISTS (SELECT * FROM sysdatabases WHERE NAME = 'ELIOD')
DROP DATABASE [ELIOD]
GO
CREATE DATABASE [ELIOD];
GO
USE [ELIOD]
GO
CREATE LOGIN [ELIOD] WITH PASSWORD='ELIOD', DEFAULT_DATABASE=[ELIOD], DEFAULT_LANGUAGE=[Spanish]
GO
EXEC master..sp_addsrvrolemember @loginame = N'ELIOD', @rolename = N'sysadmin'
GO
USE [ELIOD];
GO
CREATE USER [ELIOD] FOR LOGIN [ELIOD] WITH DEFAULT_SCHEMA=[dbo]
GO
EXEC sp_addrolemember N'db_owner', N'ELIOD'
GO
ALTER AUTHORIZATION ON SCHEMA::[db_owner] TO [ELIOD]
GO
