USE [master]
GO
IF SUSER_SID(N'ESTUDIOZ') IS NOT NULL
DROP LOGIN [ESTUDIOZ]
GO
EXEC msdb.dbo.sp_delete_database_backuphistory @database_name = N'ESTUDIOZ'
GO
ALTER DATABASE [ESTUDIOZ] SET  SINGLE_USER WITH ROLLBACK IMMEDIATE
IF EXISTS (SELECT * FROM sysdatabases WHERE NAME = 'ESTUDIOZ')
DROP DATABASE [ESTUDIOZ]
GO
CREATE DATABASE [ESTUDIOZ];
GO
USE [ESTUDIOZ]
GO
CREATE LOGIN [ESTUDIOZ] WITH PASSWORD='ESTUDIOZ', DEFAULT_DATABASE=[ESTUDIOZ], DEFAULT_LANGUAGE=[Spanish]
GO
EXEC master..sp_addsrvrolemember @loginame = N'ESTUDIOZ', @rolename = N'sysadmin'
GO
USE [ESTUDIOZ];
GO
CREATE USER [ESTUDIOZ] FOR LOGIN [ESTUDIOZ] WITH DEFAULT_SCHEMA=[dbo]
GO
EXEC sp_addrolemember N'db_owner', N'ESTUDIOZ'
GO
ALTER AUTHORIZATION ON SCHEMA::[db_owner] TO [ESTUDIOZ]
GO
...
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id('[FK_Empleado_TipoProfesion]') AND OBJECTPROPERTY(id, 'IsForeignKey') = 1)
ALTER TABLE [dbo].[Empleado] DROP CONSTRAINT [FK_Empleado_TipoProfesion];
...
ALTER TABLE [dbo].[Empleado] ADD CONSTRAINT [FK_Empleado_TipoProfesion] 
