USE [master]
GO
IF SUSER_SID(N'FJFJHHJF') IS NOT NULL
DROP LOGIN [FJFJHHJF]
GO
EXEC msdb.dbo.sp_delete_database_backuphistory @database_name = N'FJFJHHJF'
GO
ALTER DATABASE [FJFJHHJF] SET  SINGLE_USER WITH ROLLBACK IMMEDIATE
IF EXISTS (SELECT * FROM sysdatabases WHERE NAME = 'FJFJHHJF')
DROP DATABASE [FJFJHHJF]
GO
CREATE DATABASE [FJFJHHJF];
GO
USE [FJFJHHJF]
GO
CREATE LOGIN [FJFJHHJF] WITH PASSWORD='FJFJHHJF', DEFAULT_DATABASE=[FJFJHHJF], DEFAULT_LANGUAGE=[Spanish]
GO
EXEC master..sp_addsrvrolemember @loginame = N'FJFJHHJF', @rolename = N'sysadmin'
GO
USE [FJFJHHJF];
GO
CREATE USER [FJFJHHJF] FOR LOGIN [FJFJHHJF] WITH DEFAULT_SCHEMA=[dbo]
GO
EXEC sp_addrolemember N'db_owner', N'FJFJHHJF'
GO
ALTER AUTHORIZATION ON SCHEMA::[db_owner] TO [FJFJHHJF]
GO
