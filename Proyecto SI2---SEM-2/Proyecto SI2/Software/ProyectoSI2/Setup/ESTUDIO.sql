USE [master]
GO
IF SUSER_SID(N'ESTUDIO') IS NOT NULL
DROP LOGIN [ESTUDIO]
GO
EXEC msdb.dbo.sp_delete_database_backuphistory @database_name = N'ESTUDIO'
GO
ALTER DATABASE [ESTUDIO] SET  SINGLE_USER WITH ROLLBACK IMMEDIATE
IF EXISTS (SELECT * FROM sysdatabases WHERE NAME = 'ESTUDIO')
...
USE [ESTUDIO]
GO
CREATE LOGIN [ESTUDIO] WITH PASSWORD='ESTUDIO', DEFAULT_DATABASE=[ESTUDIO], DEFAULT_LANGUAGE=[Spanish]
GO
EXEC master..sp_addsrvrolemember @loginame = N'ESTUDIO', @rolename = N'sysadmin'
GO
USE [ESTUDIO];
GO
CREATE USER [ESTUDIO] FOR LOGIN [ESTUDIO] WITH DEFAULT_SCHEMA=[dbo]
GO
EXEC sp_addrolemember N'db_owner', N'ESTUDIO'
GO
ALTER AUTHORIZATION ON SCHEMA::[db_owner] TO [ESTUDIO]
GO
